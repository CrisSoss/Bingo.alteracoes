...
def on_next_number():
    ...
    if ganhadores:
        emit('winners', {'ganhadores': ganhadores}, broadcast=True)
...
# Nova função para gerar e salvar o PDF
def gerar_e_salvar_pdf(cartela, comprador, session):
    ...
    filename = f'cartela_{comprador.id}_{cartela.id}.pdf'
...
def criar_cartela():
    ...
    session.add(cartela)
    session.commit()
    gerar_e_salvar_pdf(cartela, comprador, session)
    socketio.emit('new_cartela', {'id': cartela.id})
    flash('Cartela criada com sucesso!')
    session.close()
    return redirect(url_for('index'))


@app.route('/criar_cartela_sem_info', methods=['POST'])
def criar_cartela_sem_info():
    session = Session()
    nome = "Aguardando Cadastro"
    telefone = "Aguardando Cadastro"
    endereco = "Aguardando Cadastro"
    comprador = Comprador(nome, telefone, endereco)
    session.add(comprador)
    session.commit()
    matrix = gerar_cartela()
    ...
    gerar_e_salvar_pdf(cartela, comprador, session)
    socketio.emit('new_cartela', {'id': cartela.id})
    flash(f'Cartela #{cartela.id} criada com sucesso!')
    session.close()
    return redirect(url_for('index'))


@app.route('/alterar_comprador', methods=['POST'])
def alterar_comprador():
    cartela_id = request.form['cartela_id']
    nome = request.form['nome']
    endereco = request.form['endereco']
    telefone = request.form['telefone']
    session = Session()
    ...
    if cartela:
        comprador = session.query(Comprador).get(cartela.id_comprador)
        if comprador:
            comprador.nome = nome
            comprador.endereco = endereco
            comprador.telefone = telefone
            session.commit()
            # Regenera o PDF após a alteração dos dados
            gerar_e_salvar_pdf(cartela, comprador, session)
            flash(f'Dados da cartela #{cartela_id} atualizados com sucesso!')
        else:
            flash('Comprador não encontrado.', 'error')
    else:
        flash('Cartela não encontrada.', 'error')
    session.close()
    return redirect(url_for('index'))
